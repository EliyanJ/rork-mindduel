...
/// Rork Auth (Google / Apple OAuth) manager: PKCE initiate → web session →
/// token exchange, with Keychain storage and silent refresh.
@Observable
final class AuthManager {
    ...
    /// only; routes the OAuth popup to the developer's host browser.
    ...
    /// Returns a currently valid access token, refreshing it when expired.
    func validAccessToken() async -> String? {
        if let token = KeychainHelper.get("access_token"), userFromToken(token) != nil {
            return token
        }
        await refreshToken()
    ...
    /// Decode the JWT payload for user info and expiration; the token was
    /// verified by Rork when issued.
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                ...
                if let errorResponse = try? JSONDecoder().decode(ErrorResponse.self, from: data) {
                    ...
                    setError("Échec de la connexion (\(statusCode))")
                }
                return
            }
...
nonisolated enum AuthError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noCode: return "Aucun code d'autorisation reçu"
        case .invalidURL: return "URL invalide"
        case .serverError(let code): return "Erreur serveur (\(code))"
        case .popupTimeout: return "Connexion expirée — réessaie"
        case .cancelledByUser: return "Connexion annulée"
        }
    }
}
