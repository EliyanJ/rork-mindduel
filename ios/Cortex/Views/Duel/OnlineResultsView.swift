...
struct OnlineResultsView: View {
    ...
    private var isWin: Bool { session.wonByForfeit || session.playerScore > session.opponentScore }
    private var isDraw: Bool { !session.wonByForfeit && session.playerScore == session.opponentScore }
        ...
        ScrollView {
            VStack(spacing: 22) {
                VStack(spacing: 8) {
                    ...
                    if session.wonByForfeit {
                        Text("Ton adversaire a abandonné")
    ...
    private func roundRow(index: Int, result: OnlineDuelSession.RoundResult) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 14) {
                ...
                answerBadge(
                    label: session.opponent?.name ?? "Adversaire",
