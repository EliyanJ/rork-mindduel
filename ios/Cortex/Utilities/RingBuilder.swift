...
nonisolated enum RingBuilder {
    ...
    static func rings(
        ...
        let timeline = normalizedSlots(slots, normalRingCount: groups.count)
        var rings: [PathRing] = []
        var normalCounter = 0
        var recapCounter = 0
        ...
        for slot in timeline {
            switch slot.kind {
            case .normal:
                guard let source = slot.source, groups.indices.contains(source) else { continue }
                let questions = groups[source]
                rings.append(
                    PathRing(
    ...
    /// Keeps a published timeline safe to use: drops slots pointing at rings
    /// that no longer exist, appends normal rings the layout doesn't know about
    /// (e.g. new questions were published), and guarantees a closing recap.
    private static func normalizedSlots(_ slots: [RingSlot]?, normalRingCount: Int) -> [RingSlot] {
